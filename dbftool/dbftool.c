...
void store_header_data(header *head, FILE *file, int i) {
	...
	//fread(variable donde vas a guardar, tamaño del dato que vas a leer, cantidad de datos que vas a 

leer, puntero al archivo)
...
/* This function is for parsing the field descriptors of the .dbf and loading it into a struct 

(descriptor). 
I'm using trial and error to load the correct bytes for every field.

Esta funcion es para leer el descriptor de campos del .dbf y cargarlo en una estructura de datos 
...
//search_index(campo en donde buscar, contenido que buscar, archivo que se abre, puntero al encabezado (es 

un struct), puntero al descriptor (struct))
...
int get_index(char* campo, char* string, FILE* file, header* head, descriptor* descr){
	...
	fread(buffer, descr[i].length + descr[i].decimal, 1, file);

	//avanzo en header[n].record_bytes - (descriptor[indice del campo].length + descriptor[indice del 

campo].decimal)

	fseek(file, head[0].record_bytes - (descr[i].length + descr[i].decimal) , SEEK_CUR);
