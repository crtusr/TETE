...
) 
{
  ...
  while (menu[i].text != NULL) {
    ...
    //TODO poner los items no implementados en color rojo ()
    if(!menu[i].submenu && !menu[i].action)
    {
...
int printingMenu()
{
  int start_x = 28, start_y = 8, width = 18, height = 9;
  draw_background_submenu(start_x, start_y, width, height);
  recuadro(start_x, start_y, width, height);
  int selection = 1;
  int ch = 0;
  while(ch != '\n' && ch != '\r')
  {
